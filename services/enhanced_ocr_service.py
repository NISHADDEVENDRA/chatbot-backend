...
Real PDF text extraction + OCR for image-based PDFs
"""
...
app = FastAPI(
    title="Enhanced OCR Service",
    description="Real PDF text extraction + OCR for image-based PDFs",
    version="3.0.0"
)
...
def extract_text_from_pdf_enhanced(pdf_data: bytes) -> List[Dict]:
    """Enhanced PDF text extraction with OCR for images"""
    try:
...
def process_image_with_ocr(image_data: bytes, page_num: int = 1) -> Dict:
    ...
    try:
        ...
        for config in configs:
            try:
                ...
                for i in range(len(data['text'])):
                    if int(data['conf'][i]) > 30:  # Only include text with confidence > 30%
                        text_parts.append(data['text'][i])
                        confidences.append(int(data['conf'][i]))
                
                if text_parts:
                    current_text = ' '.join(text_parts).strip()
                    current_confidence = sum(confidences) / len(confidences) / 100.0  # Convert to 0-1 scale
        ...
        if not extracted_text:
            try:
                ...
                extracted_text = f"[OCR processing failed for image on page {page_num}]"
                best_confidence = 0.1
...
def calculate_quality_score_enhanced(text: str) -> float:
    """Enhanced quality scoring for OCR text"""
...
async def extract_text(
    ...
    """Enhanced text extraction with OCR for image-based PDFs"""
    start_time = time.time()
    ...
    try:
        ...
        if "pdf" in file_type.lower():
            result = await process_pdf_enhanced(file_data, extract_tables, extract_images, confidence_threshold)
        elif "image" in file_type.lower():
            result = await process_image_enhanced(file_data, extract_tables, extract_images, confidence_threshold)
        else:
...
async def process_pdf_enhanced(pdf_data: bytes, extract_tables: bool, extract_images: bool, confidence_threshold: float) -> Dict:
    """Enhanced PDF processing with OCR"""
    try:
        # Extract pages data from PDF
        pages_data = extract_text_from_pdf_enhanced(pdf_data)
            ...
            if extract_images and page_data["images"]:
                ...
                for img_data in page_data["images"]:
                    try:
                        ...
                        ocr_result = process_image_with_ocr(img_bytes, page_num)
        ...
        quality_score = calculate_quality_score_enhanced(all_text)
        language = detect_language_enhanced(all_text)
...
async def process_image_enhanced(image_data: bytes, extract_tables: bool, extract_images: bool, confidence_threshold: float) -> Dict:
    """Enhanced image processing with OCR"""
    try:
        result = process_image_with_ocr(image_data, 1)
        
        text = result["text"]
        word_count = len(text.split())
        character_count = len(text)
        quality_score = calculate_quality_score_enhanced(text)
        language = detect_language_enhanced(text)
...
if __name__ == "__main__":
    ...
    logger.info(f"Starting Enhanced OCR service on {host}:{port}")
    logger.info("Using PyMuPDF + Tesseract for real PDF text extraction")
